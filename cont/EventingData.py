    ...
    if PlotVizSelFDData == "Passes":
        ...
        with pltmain11:
            ...
            if OptionPlotSel == 'Passes Viz':
                ...
                ax.text(52.5,70, "" + PlayerSelExpData_txt.upper() + " - " + str(len(dfKKK)) + " PASES COMPLETOS", c='w', fontsize=10, fontproperties=prop2, ha='center')
            ...
            if OptionPlotSel == 'Progressive Passes':
                ...
                ax.text(52.5,70, "" + PlayerSelExpData_txt.upper() + " - " + str(len(dfprog)) + " PASES PROGRESIVOS", c='w', fontsize=10, fontproperties=prop2, ha='center')
            ...
            if OptionPlotSel == 'Passes Into Final Third':
                ...
                ax.text(52.5,70, "" + PlayerSelExpData_txt.upper() + " - " + str(len(dfKKK)) + " PASES HACIA ÚLTIMO TERCIO", c='w', fontsize=10, fontproperties=prop2, ha='center')
                ax9 = fig.add_axes([0.20,0.14,0.63,0.07])
                ax9.set_xlim(0,105)
                ax9.set_ylim(0,20)
                ax9.axis("off")
            ...
            if OptionPlotSel == 'Passes Into Penalty Area':
                ...
                ax.hlines(xmin=88.5, xmax=105, y=13.84, color='w', alpha=1, ls='--', lw=2, zorder=-1)
                ax.add_patch(Rectangle((88.5, 13.84), 16.5, 40.32, fc="#000000", fill=True, alpha=0.7, zorder=-2))
                ax.text(52.5,70, "" + PlayerSelExpData_txt.upper() + " - " + str(len(dfKKK)) + " PASES HACIA ÁREA RIVAL", c='w', fontsize=10, fontproperties=prop2, ha='center')
                ax9 = fig.add_axes([0.20,0.14,0.63,0.07])
                ax9.set_xlim(0,105)
                ax9.set_ylim(0,20)
                ax9.axis("off")
            ...
            if OptionPlotSel == "Passes Into Half Spaces":
                ...
                ax.text(52.5,70, "" + PlayerSelExpData_txt.upper() + " - " + str(len(df)) + " PASES HACIA ESPACIOS INTERMEDIOS", c='w', fontsize=10, fontproperties=prop2, ha='center')
                ax9 = fig.add_axes([0.20,0.14,0.63,0.07])
                ax9.set_xlim(0,105)
                ax9.set_ylim(0,20)
                ax9.axis("off")
            ...
            if OptionPlotSel == 'Long Passes':
                ...
                pitch.lines(dff['X1'], dff['Y1'], dff['X2'], dff['Y2'], cmap=get_continuous_cmap(hex_list), ax=ax, lw=2, comet=True, transparent=True, zorder=3) 
                ax.scatter(dff['X2'], dff['Y2'], color="#9F9F9F", edgecolors='#121214', zorder=3, lw=0.5)
                ax.text(52.5,70, "" + PlayerSelExpData_txt.upper() + " - " + str(len(df)) + " PASES LARGOS", c='w', fontsize=10, fontproperties=prop2, ha='center')
                ax9 = fig.add_axes([0.20,0.14,0.63,0.07])
                ax9.set_xlim(0,105)
                ax9.set_ylim(0,20)
                ax9.axis("off")
